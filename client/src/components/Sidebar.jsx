...
const Sidebar = () => {
    const {guestUsers, user, setSelectedUser, selectedUser, unseenMessages, setUnseenMessages} = useContext(ChatContext);
    const {logout, onlineUsers} = useContext(AuthContext);
    const [input, setInput] = useState('');

    const navigate = useNavigate();
    const filterUsers = user ? user.filter((u) => u.fullName.toLowerCase().includes(input.toLowerCase())) : [];
        ...
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden": ''} `}>
            ...
            <div className='flex flex-col'>
                {filterUsers.length > 0 && filterUsers.map((user, index) => (
                    <div onClick={() => {
                        // Corrected: Use a semicolon to separate the function calls
