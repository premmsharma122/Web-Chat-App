...
const ChatContainer = () => {
    ...
    const handleSendMessage = async (e) => {
        ...
        await sendMessage({ text: input.trim(), receiver: selectedUser });
        setInput('');
    };
    ...
    const handleSendImage = async (e) => {
        ...
        reader.onload = async () => {
            await sendMessage({ image: reader.result, receiver: selectedUser });
            e.target.value = '';
        };
        ...
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
                {messages.map((msg, index) => (
                    <div className={`flex items-end gap-2 ${msg.senderId !== authUser._id ? 'justify-start' : 'justify-end'}`} key={index}>
                        {msg.image ? (
                            <img className='max-w-[230px] border border-b-gray-700 rounded-lg overflow-hidden mb-8' src={msg.image} alt="" />
                        ) : (
